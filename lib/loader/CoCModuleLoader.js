...
class CoCModuleLoader {
    ...
    registerModules(modules) {
        return this.Promise.each(modules, ({module, priority}) => {
            return this.registerModuleRoot(this.moduleToRoot(module), priority, false);
        })
            .then(() => {
                return this.resortModuleRoots();
            });
    ...
    registerModule(module, priority = 0) {
        return this.registerModuleRoot(this.moduleToRoot(module), priority, true);
    }
    ...
    load(moduleId) {
        ...
        if (match) {
            ...
            return this.categoryPlugins.get(loader)(environment, pathSpec)
                .then((modulePath) => {
                    return this.Promise.resolve(this.require(modulePath))
                });
        }
        else {
    ...
    moduleToRoot(module) {
        return pathModule.dirname(this.require.resolve(module));
    }
