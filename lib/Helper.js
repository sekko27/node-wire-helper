...
class Helper {
    static isSecret(key) {
        ...
        return normalized.test(/secret/)
            || normalized.test(/token/)
            || normalized.test(/api_key/)
            || normalized.test(/apikey/);
    }
    ...
    static envVar(name, def) {
        if (process.env[name]) {
            ...
            const displayValue = Helper.isSecret(name) ? '[HIDDEN]' : value;
            console.log(`Using ${name} environment variable: ${displayValue}`);
            return value;
        } else {
            const displayValue = Helper.isSecret(name) ? '[HIDDEN]' : def;
    ...
    static envIntegerList(name, delimiter = ',', def = []) {
        const list = _.chain(Helper.envList(name, delimiter, def))
